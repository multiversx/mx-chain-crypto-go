...
import (
	...
	plonk_verifier "github.com/multiversx/mx-chain-crypto-go/zk/plonk"
...
func TestVerifyPlonk(t *testing.T) {
	...
	verified, err := plonk_verifier.VerifyPlonk(uint16(ecc.BLS12_381), serializedProof.Bytes(), serializedVK.Bytes(), pubWBytes)
	require.True(t, verified)
	require.Nil(t, err)

	// Invalid proof
	verified, err = plonk_verifier.VerifyPlonk(uint16(ecc.BLS12_381), []byte{}, serializedVK.Bytes(), pubWBytes)
	require.False(t, verified)
	require.Error(t, err)

	// Invalid public witness
	verified, err = plonk_verifier.VerifyPlonk(uint16(ecc.BLS12_381), serializedProof.Bytes(), serializedVK.Bytes(), []byte{})
	require.False(t, verified)
	require.Error(t, err)

	// Invalid vk
	verified, err = plonk_verifier.VerifyPlonk(uint16(ecc.BLS12_381), serializedProof.Bytes(), []byte{}, pubWBytes)
	require.False(t, verified)
	require.Error(t, err)
	...
	_, _ = plonk_verifier.VerifyPlonk(uint16(ecc.UNKNOWN), serializedProof.Bytes(), serializedVK.Bytes(), pubWBytes)
}
